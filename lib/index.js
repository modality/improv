...
class Improv {
  ...
  wrappedGen = (...rest) => {
    if (typeof this.__wrappedGen === 'undefined') {
      ...
      this.__wrappedGen = middlewares.reduce(
        reducer.bind(this),
        this.__gen
      );
    }
  ...
  __gen = (snippet, model, subModelName) => {
    ...
    */
    ...
    const chosenPhrase = this.selectPhrase(
      this.scoreFilter(filteredGroups),
      model
    );
  ...
  validateSnippetDefinition = (snippetName) => {
    ...
    const {groups} = this.snippets[snippetName];
    if (!Array.isArray(groups)) {
      throw new TypeError(
        `Missing or bad groups array for snippet ${snippetName}; was ${typeof groups}`
      );
    }
  }
  ...
  selectPhrase = (groups, model) => {
    ...
    */
    ...
    if (phrases.length === 0) {
      ...
      throw new Error(
        `Ran out of phrases in ${groups} while generating ${
          this.__currentSnippet
        }`
      );
    }
  ...
  scoreFilter = (groups) => {
    ...
    */
    ...
    const maxScore = validGroups.reduce(
